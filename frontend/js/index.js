...
// dans ce type de code, les action piocher, defausser, etc...
...
// logique du jeu : condiion de victoire, etc
  ...
  if (ennemyPlayer.socketId !== player.socketId && !ennemyPlayer.turn) {
    ...
    if (ennemyPlayer.win) {
      setTurnMessage(
        "alert-info",
        "alert-danger",
        `C'est perdu ! <b>${ennemyPlayer.username}</b> a gagné !`,
      );
    ...
    setTurnMessage(
      "alert-success",
      "alert-info",
      `C'est au tour de <b>${ennemyUsername}</b> de jouer`,
    );
    player.turn = false;
  }
});
